...
export default async function handler(req, res) {
  ...
  // Health check: open /api/generate in the browser to diagnose problems.
  if (req.method === "GET") {
  ...
  if (!apiKey) {
    return res.status(500).json({ error: "ANTHROPIC_API_KEY is not set on the server. Add it in Vercel env vars and redeploy." });
  }

  // req.body may arrive as a string depending on the runtime
  let body = req.body;
...
Make it short, genuinely funny or vivid, and actually useful for remembering — not generic.`;
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return res.status(502).json({ error: `Anthropic API ${response.status}: ${detail}` });
    }
